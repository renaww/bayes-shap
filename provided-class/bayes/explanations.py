...
class BayesLocalExplanations:
    ...
    """
    --------------------------------------------------------------------------
    Focus sampling for SHAP regression
    (Choose which coalitions to evaluate next based on uncertainty in regression)
    --------------------------------------------------------------------------
    """
    def shap_tabular_focus_sample_regress(self,
                                      data,
                                      regress_f,
                                      n_samples,
                                      focus_sample_batch_size,
                                      focus_sample_initial_points,
                                      to_consider=10_000,
                                      temperature=1e-2, # Lower temperature--exploit high variance areas
                                      enumerate_initial=True):
        assert focus_sample_initial_points > 0, "Initial focusing sample points cannot be <= 0"
        current_n_perturbations = 0
        # Get 1's coalitions, if requested
        ...
        if current_n_perturbations < focus_sample_initial_points:
            initial_perturbations = self._shap_tabular_perturb_n_samples(
        ...
        # Store initial predictions
        # ADAPETD FOR 1D REGRESSION OUTPUT
        current_perturbations.append(regress_f(current_perturbations[SINVERSE]))
        if self.verbose:
            pbar.update(current_perturbations[SDATA].shape[0])
        while current_n_perturbations < n_samples:
            current_batch_size = min(focus_sample_batch_size, n_samples - current_n_perturbations)

            # Fit BLR on current data with SHAP kernel weights
            blr = BayesianLinearRegression(percent=self.cred_int)
            weights = self._get_shap_weights(current_perturbations[SDATA],
                                            M=current_perturbations[SDATA].shape[1])
            blr.fit(current_perturbations[SDATA], current_perturbations[SY], weights, compute_creds=False)

            # Propose candidates and score by predictive variance
            candidate_perturbations = self._shap_tabular_perturb_n_samples(data, to_consider)
            _, var = blr.predict(candidate_perturbations[SINVERSE])

            # Temperature softmax over variance
            var /= temperature
            exp_var = np.exp(var)
            temperature_scaled_weights = exp_var / np.sum(exp_var)

            # Sample indices (with replacement to emphasize high-variance areas)
            chosen = np.random.choice(len(var), size=current_batch_size,
                                    p=temperature_scaled_weights, replace=True)

            # Label chosen candidates using the regressor
            cy = regress_f(candidate_perturbations[SINVERSE][chosen])

            new_perturbations = self._select_indices_from_data(candidate_perturbations, chosen, cy)
            current_perturbations = self._stack_tabular_return(current_perturbations, new_perturbations)
            current_n_perturbations += new_perturbations[0].shape[0]
    ...
    """
    --------------------------------------------------------------------------
    BayesSHAP (tabular) explanations (combines explanation function from below)
    Replaces classifier_f with regress_f for regression tasks
    --------------------------------------------------------------------------
    """
    def explain_bayes_shap_regression(self,
                            data,
                            regress_f, # Regression instead of classification. 1D
                            label = None, #ignore
                            cred_width=1e-2,
                            focus_sample=True,
                            n_samples=None,
                            ...
                            max_coefs=None,
                            l2=True,
                            feature_select_num_points = 1000):
        # Same point definitions as general explain function
        ...
        if n_samples is not None and not focus_sample:
            ...
            y = regress_f(inverse)[:, label]
            weights = self._get_shap_weights(rdata, M=rdata.shape[1])

            blr = BayesianLinearRegression(percent=self.cred_int)
            print("rdata shape:", rdata.shape)
            print("y shape:", y.shape)
            reshaped_y = np.asarray(y).reshape(-1)
            print("reshaped y shape:", reshaped_y.shape)
            blr.fit(rdata, reshaped_y, weights)

        # Focused sampling case (Use BLR variance)    
        elif focus_sample:
            if feature_selection:
                raise NotImplementedError
            logging.info("Starting focused sampling")
            if n_samples:
                logging.info("n_samples preset, running focused sampling up to %s samples", n_samples)
                logging.info("using batch size %s with %s initial points", focus_sample_batch_size, focus_sample_initial_points)
                focused_sampling_output = self.shap_tabular_focus_sample_regress(data,
                                                                         regress_f,
                                                                         n_samples,
                                                                         focus_sample_batch_size,
                                                                         focus_sample_initial_points,
        ...
        # Use PTG to get initial samples
            ...
            # Estimate additional samples needed, gather and refit
            n_needed = int(np.ceil(blr.get_ptg(cred_width)))
            ptg_rdata, ptg_inverse  = self._shap_tabular_perturb_n_samples(data, 
                                                                           n_needed - ptg_initial_points,
                                                                           max_coefs)
        ...
        output = {
            "data": rdata,
            "y": y,
            "distances": weights, # kernel weights
            "blr": blr,
            "coef": blr.coef_,
            "max_coefs": max_coefs
        }
